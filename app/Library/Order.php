...
class Order
{
    ...
    public function getAttachedProducts($id)
    {
        return DB::table('Product AS p')
            ->select(["p.*", "pc.TariffCode", "i.CountryID", "i.Note", "c.Name AS CountryName", "c.CodeName2 AS CountryCodeName"])
    ...
    public function getDeliveryCompanySettings($deliveryCompany = false)
    {
        ...
        if (isset($deliverySettings[$deliveryCompany]) && !empty($deliverySettings[$deliveryCompany])) {
    ...
    {
        return Client::selectRaw("Client.*, c.CodeName2 AS 'CountryCodeName', c.Name AS 'CountryName', c.RegionID")
            ->leftJoin('Country AS c', 'Client.CountryID', '=', 'c.CountryID')
