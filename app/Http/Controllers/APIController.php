...
class APIController extends Controller
{
    ...
    public function shipmentValidationDHL($id)
    {
        ...
        if ($settings = ClientShipping::where('client_id', $order->ClientID)->first()) {
            $deliveryCompanySettings = $settings->content['dhl'];
        } else if ($settings = $this->order->getDeliveryCompanySettings('dhl')) {
            $deliveryCompanySettings = $settings;
        } else {
            return $this->sendError('API configuration error');
        }
        ...
        if ($order->ClientID == 10) {
            $accountNumber = '425978591';

            $shipper->CompanyName = 'ZM Technologies Limited';
            $shipper->Address1 = 'Industrieweg 4';
            $shipper->Address2 = '';
            $shipper->Address3 = '';
            $shipper->Address4 = 'Maarssen';
            $shipper->Postcode = '3606AS';
            $shipper->Email = 'info@apotheeklife.com';

            $vat = 'GB423627604';
            $billedTo = 'GB423627604';
            $eori = 'GB423627604000';
            //$phone = preg_replace('/[^0-9]/', '', '0850644162');
        } else if ($order->ClientID != 8) {
            $accountNumber = '425781155';

            $shipper->CompanyName = 'Apotheek Life BV';
            $shipper->Address1 = 'Industrieweg 4';
            $shipper->Address2 = '';
            $shipper->Address3 = '';
            $shipper->Address4 = 'Maarssen';
     ...
     *
     * @param int $id
     * @return Illuminate\Http\Response|Throwable
     */
    ...
    public function getLabelDPD($id)
    {
        ...
        if (isAzureStorageEnabled()) {
            return downloadRemoteFile('dpd_labels/' . $id . '.epl', $id . '.epl');
        } else {
            header("Content-Type: text/zpl;encoding=UTF-8");
            header("Content-Disposition: inline; filename=$id.zpl");
