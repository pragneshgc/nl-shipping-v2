        ...
        if ($data) {
            ...
            foreach ($products as $product) {
                ...
                if (!$product->AttachedProducts->isEmpty()) {
                    $totalDosage = 0;
                    $attachedItems = count($product->AttachedProducts);
                    foreach ($product->AttachedProducts as $attached) {
                        $totalDosage += $attached->Quantity;

                        if ($attached->Status != 'SHIPPED') {
                            $notshipped++;
                        } else if ($attached->Status == 'SHIPPED') {
                            $shippedItems++;
                        }
                    }

                    if ($totalDosage >= ($product->Quantity * $product->Dosage)) {
                        $data->CanDecomission = true;
                    } else {
                        $data->CanDecomission = false;
                    }
                } else {
                    $notshipped++;
                }
            ...
            if ($shippedItems <= $attachedItems) {
                $data->canPrintLabel = true;
            } else {
                $data->canPrintLabel = false;
            }
