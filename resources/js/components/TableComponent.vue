...
export default {
    ...
    setup(props) {
        ...
        // Watchers
        watch(() => _.debounce(function () {
            data.value.current_page = 1;
            console.log('debounce');
            getData();
        }));
        watch(() => props.$route.query, () => {
            console.log('route query');
            if (typeof props.$route.query.q != 'undefined') {
                queryString.value = props.$route.query.q;
            } else {
                queryString.value = '';
            }
            if (typeof props.$route.query.p != 'undefined') {
                data.value.current_page = props.$route.query.p;
        ...
        watch(
            range,
            () => {
                console.log('range watcher 2', range);
            },
            { immediate: true }
        );
        watch(limit, () => {
        ...
        watchEffect(() => {
            console.log('range effect', range.value);
        })

        /* watch([() => queryString.value, () => range.value, () => limit.value, () => props.generalQuery, () => props.filters,
        () => props.$route.query],
            () => {
                data.value.current_page = 1;
                getData();
